...
static void stateMachineWork(){
    ...
    switch(event_to_process){
        ...
        case 10: // B2_POINT_OF_INTEREST_FIN_A
            // EXECUTE SOME MOVIMENTS RELATED TO NON-MODELED EVENTS
            gpio_set_level(25, 1);
            ESP_LOGI(TAG, "Event abstrated from B2_POINT_OF_INTEREST_FIN_A is processing!");
            vTaskDelay(5000 / portTICK_PERIOD_MS);

            // SEND THE EVENT OF END THAT IS UNCONTROLLABLE
            strcpy(event_to_update, "B2_POINT_OF_INTEREST_FIN_A");
            gpio_set_level(25, 0);
        ...
        case 11: // B2_POINT_OF_INTEREST_FIN_B
            // EXECUTE SOME MOVIMENTS RELATED TO NON-MODELED EVENTS
            gpio_set_level(25, 1);
            ESP_LOGI(TAG, "Event abstrated from B2_POINT_OF_INTEREST_FIN_B is processing!");
            vTaskDelay(5000 / portTICK_PERIOD_MS);

            // SEND THE EVENT OF END THAT IS UNCONTROLLABLE
            strcpy(event_to_update, "B2_POINT_OF_INTEREST_FIN_B");
            gpio_set_level(25, 0);
        ...
        case 12: // B2_MAINTENANCE
            // EXECUTE SOME MOVIMENTS RELATED TO NON-MODELED EVENTS
            gpio_set_level(32, 0);
            gpio_set_level(25, 1);
            ESP_LOGI(TAG, "Event abstrated from B2_MAINTENANCE is processing!");
            vTaskDelay(5000 / portTICK_PERIOD_MS);

            // SEND THE EVENT OF END THAT IS UNCONTROLLABLE
            strcpy(event_to_update, "B2_MAINTENANCE");
            gpio_set_level(25, 0);
            gpio_set_level(32, 1);
...
static void http_get_task(void *pvParameters)
{
    ...
    while(1) {
        ...
        if(state_machine == 0){
            ...
            if((strcmp(event_to_update, "B2_FIN_A_END") == 0) && (state_machine == 0)){
                state_machine = 3;
                event_to_process = 10;
            }else if((strcmp(event_to_update, "B2_FIN_B_END") == 0) && (state_machine == 0)){
                state_machine = 3;
                event_to_process = 11;
            }else if((strcmp(event_to_update, "B2_SUPPORT_END") == 0) && (state_machine == 0)){
                state_machine = 3;
                event_to_process = 12;
            }
